...
[Jawbone UP24/UP3](https://jawbone.com) and [Microsoft Band 2](https://microsoftband.com/)  are two of the most popular fitness trackers in the world.   
...
Concerning usability of current applications - it is user-friendly and also allows users to log  food consumption to help with their dietary goals.
...
<div style='text-align: bottom;'>
  <img text="Fig" widht='350' height='100' align='left' src='assets/img/MicrosoftBand2.jpg'/>
</div>
This HT was not tested, but it it deserves attention and may be present the best desition. The Microsoft Band is a commercial product available to the public without any delays and problems. It features big number of important sensors: optical heart rate sensor, 3-axis accelerometer/gyrometer, ambient light sensor, UV sensor, capacitive sensor, galvanic skin response, baromete, haptic vibration motor, microphone, GPS. But the big positive point is that Microsoft has provided a SDK that allows access of the band’s sensor readings. The band communicates via Bluetooth to a synchronized smart phone. 
So it can present a good solution as it takes access to sensors data like Angel Sensor and has user-friendly application like Jawbone, which is also important for motivating. 
But the price is higher approximately on 100$. 

### Energy expenditure estimation

Using parameters from HT we can measure and control the physical parameters, intencity of activity  and energy expenditure (EE). And as calories burnt (EE) estimation present the key aspect in obesity determination, this section we dedicate to consept of EE measurement. 
We quickly  investigate existing problems of EE estimation. Then  the relation between accelerometer, physiological data and EE  will be establised.
And at the end we propose models which will be able to increase accuracy level and to take into account variability in physiological signals between individuals applaying method of personalization. 
...
Metabolic carts offer invaluable insight into exercise metabolism, allowing interrogation of each breath. 
There are a couple of portable indirect calorimeters on the market, like AeroSport TEEM 100 Metabolic Analysis System,  but the participant must use a facial mask during all data acquisition. 
...
data(gxtmax)
data(gxtsub)
# converting these data sets to the clean-printing `tbl` format of `dplyr` for the purposes of this demo
gxtmax <- as.tbl(gxtmax)
gxtsub <- as.tbl(gxtsub)
#cleaning data
#Errant values
## Exclude those breaths that exhibit an oxygen uptake (VO2)
## > 3 standard deviations  from the local 30 breath mean.
#gxtmax %$% errant(vo2, method = "outlier", breaths = 30) %>% which
#remove errant value
gxtmax %<>% subset(!errant(vo2, "outlier"))
gxtsub %<>% subset(!errant(vo2, "outlier"))
#The removal of ventilatory and cardiovascular frequency components---leaving theoretically only the #muscular oxygen uptake kinetics---via the application of a 0.04 Hz, low-pass, third-order Butterworth #filter.
gxtmax %<>% mutate(vo2_smth = vo2_butter(time, vo2, zerolag = FALSE))
# Substrate utilisation
gxtsub %<>%  
  ## Convert gas exchange values to L/min.
  mutate_each(funs(. / 1000), matches("^v.*2$")) %>%
  bind_cols(metab(vo2, vco2, data = ., as.ee = TRUE))
gxtsub %>% 
  ## Drop unneccessary columns and convert to long format.
  select(-matches("v|2")) %>% 
  tidyr::gather(key, value, -time, -wr, factor_key = TRUE) %>% 
  filter(wr < 260) %>%
  ## Plot.
  ggplot(aes(x = factor(wr), y = value), Caption="") + geom_boxplot() +
  facet_wrap(~key) + 
  labs(x = "Power (Watts)", y = "Energy Expenditure (kJ/min)", 
       title = "Submaximal substrate utilisation")
...
```{r echo = F, fig.height = 3}
gxtsub %$% fatmax(fatox, x = wr)  # VO2 could also be used.

## Plot.
gxtsub %>% ggplot(aes(x = wr, y = fatox * 1000)) +
  stat_summary(fun.data = mean_cl_normal) +
  geom_smooth(method = "lm", formula = y ~ poly(x, 2)) +
  geom_vline(aes(xintercept = fatmax(fatox, wr)), linetype = 3) +
  labs(x = "Power (Watts)", y = "Fat Oxidation (J/min)", 
       title = expression(Fat[max]))
```

And  at the end,  we can try to evalue another important  parameter -  efficiency during exercise.
The two dominating methods are calculations of gross *efficiency* and *delta* efficiency, were described in this work[46].

```{r echo=F, fig.height=3}

gxtsub %<>% mutate(ee.W = ee * 1000 / 60, GE = wr / ee.W)

## Produce the model.
mod_ge <- nls(GE ~ A * (1 - exp(-wr / C)), data = na.omit(gxtsub), 
              start = list(A = 0.3, C = 200))

## Plot.
na.omit(gxtsub) %>% ggplot(aes(x = wr, y = GE)) + 
  geom_jitter(aes(colour = factor(wr)), size = 0.7) +
  stat_summary(fun.data = mean_cl_normal) + 
  stat_smooth(method = "nls", formula = y ~ A * (1 - exp(-x / C)), se = FALSE,
              method.args = list(start = list(A = 0.3, C = 200))) +
  guides(colour = FALSE) +
  labs(x = "Work rate (Watts)", 
       y = "Gross efficiency (%)", 
       title = sprintf("Gross efficiency: ~%.3f", coef(mod_ge)["A"]))

# Delta efficiency --------------------------
## Produce the model.
mod_de <- lm(wr ~ ee.W, data = na.omit(gxtsub))

## Plot.
na.omit(gxtsub) %>% ggplot(aes(x = ee.W, y = wr)) +
  geom_jitter(aes(colour = factor(wr)), size = 0.7) +
  geom_smooth(method = "lm") + 
  guides(colour = FALSE) +
  labs(x = "Metabolic energy expenditure (Watts)",
       y = "Work rate (Watts)", 
       title = sprintf("Delta efficiency: ~%.3f", coef(mod_de)[2]))
...
For improving accelerometer data almost all modern devices and many researches (like [40], [41]) try to use activity recognition algorithms, where activity recognition is performed in different way. For example, it depends on sensor location,  the number and types of clusters in set: it could classify 5 or 50 activities with different levels of accuracy, as well as  the set  can be mutually exclusive. 
...
For resolving this problems some researches propose the following possiable ways:
...
library(qtl)
#data(hyper)
#hyper <- sim.geno(obesity.cross, step=2, n.draws=16, error.prob=0.01)
...
40. V. T. van Hees, R. C. van Lummel, and K. R. Westerterp. Estimating Activity-related Energy Expenditure Under Sedentary Conditions Usinga Tri-axial Seismic Accelerometer. Obesity, 17(6):1287–1292, 2009.
41. A. G. Bonomi. Improving assessment of daily energy expenditure by identifying types of physical activity with a single accelerometer. Journal 
of Applied Physiology, 107(3):655-661, 2009.
42.  M. Altini, J. Penders, and O. Amft. Personalizing energy expenditure estimation using a cardiorespiratory fitness predicate. In Pervasive Computing Technologies for Healthcare (PervasiveHealth), 2013 7th International Conference on, pages 65-72. IEEE, 2013.
43. M. Altini, P. Casale, J. Penders, O. Amft. "Personalized cardiorespiratory fitness and energy expenditure estimation using hierarchical bayesian models" accepted for publication in the Journal of biomedical informatics. 2015.
44. Jeukendrup AE, Wallis GA. Measurement of Substrate Oxidation During Exercise by Means of Gas Exchange Measurements. International Journal of Sports Medicine 26: S28–S37, 2005.
45. Robergs RA, Dwyer D, Astorino T. Recommendations for improved data processing from expired gas analysis indirect calorimetry. Sports Medicine 40: 95–111, 2010.
46. Ettema G, Lorås HW. Efficiency in cycling: a review. Eur J Appl Physiol 106: 1–14, 2009.