...
As result the maximum of accuracy has appeared around 490 variables and has made 0.9545, that is better on quarter of percent than previous value  from full set of variables.

**`Second approach: important variables with Information gain ratio`** 

For being determined in choice of important variable, we apply Information gain ratio (IGR).  After calculation of IGR,  the list of variables was arranged in decreasing order of value. The first 20 from this list is  following:
```{r echo=F}
if (switch_activity == 1){
  vars <-.activityIGR()
}  
...
The difference of  resulatats  could be explained by the fact that in dataset some variables give more information than others. It is well visible if to construct diagrams of variables with the different IGR value. For example we have selectes the two variables in two opposite cases: in case of high IGR the  points are  visually separated over against  low IGR case, which  is also carry  some portion of useful information.
![Fig. Example IGR = 0.88](assets/img/exmpl_igr_hight.png)
![Fig. Example IGR = 0.11](assets/img/exmpl_igr_low.png)
Finally number of classes also plays some role in it, because  reaching the maximum accuracy in the one class, we can lose performance in another. This can be shown on the discrepancy matrices for two different sets of attributes with the same precision:


**`Control: RF model (IGR 526 variables)`**

```{r echo=F}

#Having received the arranged list of important variables with IGR  we apply them for #RF and SVM: 
if (switch_activity == 1){
  .activityRFigr(vars)
}  

.printReference("data/conf_matrix(igr_val)RF.xlsx")

#.printOverall("data/conf_matrix(igr_val)RF.xlsx")
 

```

**`Control: SVM model (547  IGR variables)`**
```{r echo=F}
if (switch_activity == 1){
  .activitySVMigr(vars)
}
  
.printReference("data/conf_matrix(igr_val)SVM.xlsx")

#.printOverall("data/conf_matrix(igr_val)SVM.xlsx")
...
As it is shown in the tables, accuracy of SVM model is higher then RF.
...
This test demonstrates that in case of human activity recognition SVM model works faster  and shows a little bit better accuracy than RF model. The lists of important variables by RF and IGR  raise the question of whether to use the gyroscope to improve recognition accuracy.   
...
Singular value decomposition (SVD) is a factorization method that decomposes the data into a set of mutually orthogonal "eigentraits" that are sorted according to variance explained [10].  One benefit of this process is that variants that are weakly correlated to several phenotypes due to common underlying processes may be strongly correlated to one of the eigentraits. This eigentrait captures the information of the underlying process, making strong main effects distributed between phenotypes easier to detect and identify as potential interaction loci and/or covariates.
...
```
![Fig. The results of the single-locus scan ](assets/img/singleScane.png)
...
But before running the pair scan, it is important that we reduce the genetic matrix to only markers that are linearly independent of one another. We need calculate the correlation between all pairs of markers and  in case of founding pair perfectly
correlated, the first marker is removed. 
```{r echo=F, fig.height=6}
if (switch_cape == 1){
...
```
![Fig. The results of the  markers selected for pairscan](assets/img/singleScane.png)

On figure we can see the locations of discarded markers, which markers have been selected for the pair scan and which have been rejected.
...
This plot shows the resulting β/σ for each pair of markers for ET1. Gray and white bars show the boundaries of the chromosomes.
**(to be continued)**

### Detection of Interactions

Below we present plot of Variant influences, which is useful for identifying patterns in variant interactions and shows all significant influences between variants and between variants and phenotypes.
```{r echo=F}
if (switch_cape == 1){
  ...
  standardize = TRUE, not.tested.col = "lightgray", neg.col = "yellow", pos.col = "green",pheno.width = 8)
  dev.off()
...
```
![Fig. Plot Network](assets/img/network.png)
...
circles outside the chromosome bars. Green segments represent significant positive main effects and yellow segments represent significant negative main effects. Interactions are denoted by arrows between chromosomal regions.
